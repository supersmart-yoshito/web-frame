...
<table id="message">
  ...
  <tr>
    ...
    <td><a href="/message/outbox/{$message->getId()}">{$message->getUpdated()|date_format:'%Y/%m/%d %H:%M'}</a></td>
    <td><a href="/message/outbox/{$message->getId()}">{$message->getToAccountId()}</a></td>
    <td><a href="/message/outbox/{$message->getId()}">{$message->getSubject()|truncate:30}</a></td>
  </tr>
  {foreachelse}
  <tr class="h300">
  <td>メッセージはありません</td>
